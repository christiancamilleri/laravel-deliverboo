...
<div class="container px-5 my-5" >
    ...
    <form action="{{ route('admin.restaurants.update', $restaurant) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="my-5 form-check" >
            ...
            <input class="form-control @error('name') is-invalid @enderror" type="text" id="name" name="name" value="{{old('name') ?? $restaurant->name}}" required>
            @error('name')
        ...
        <div class="my-5 form-check" >
            ...
            <input class="form-control @error('address') is-invalid @enderror" type="text" id="address" name="address" value="{{old('address') ?? $restaurant->address}}" required>
            @error('address')
            <div class="invalid-feedback">
                <em> {{$message}} </em>
            </div>
        ...
        <div class="my-5 form-check" >
            ...
            <input class="form-control @error('postal_code') is-invalid @enderror" pattern="[0-9]{5}" type="text" id="postal_code" name="postal_code" value="{{old('postal_code') ?? $restaurant->postal_code}}" required>
            @error('postal_code')
            <div class="invalid-feedback">
                <em> {{$message}} </em>
            </div>
        ...
        <div class="my-5 form-check">
            ...
            <input class="form-control @error('vat_number') is-invalid @enderror" type="text" style="text-transform: uppercase;" pattern="[A-Za-z]{2}[0-9]{11}" id="vat_number" name="vat_number" value="{{old('vat_number') ?? $restaurant->vat_number}}" required>
            @error('vat_number')
            <div class="invalid-feedback">
                <em> {{$message}} </em>
            </div>
        ...
        <div class="form-group form-check my-5 form-group d-flex gap-3 align-items-center">
            ...
            <div class="row">
                ...
                <div class="form-check col-3 d-flex align-items-center gap-2">
                    ...
                    @else
                    <input type="checkbox" id="typology-{{$typology->id}}" name="typologies[]" value="{{$typology->id}}" @checked($restaurant->typologies->contains($typology))>
                    @endif
